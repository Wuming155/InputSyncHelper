...
* **⚙️ 自定义设置**：支持自定义局域网IP、端口号、窗口大小和退格限制，满足不同网络环境和个人需求。
...
3. **扫码连接**：使用手机浏览器（推荐 Chrome/Safari）扫描二维码，或直接在手机浏览器中输入显示的链接。
4. **开始创作**：在手机输入框内打字或使用语音，文字将实时录入电脑光标位置。
5. **自定义设置**：点击左上角的"自定义"按钮，可设置局域网IP、端口号、窗口大小和退格限制。设置保存后会自动应用并重启服务器。
