            ...
            if msg.type == aiohttp.WSMsgType.TEXT:
                ...
                if data.get('type') == 'config':
                    ...
                    if pending_strip_punctuation and d_cnt == 0 and add_txt:
                        punc = "，。、；：？！""''·…—～,.;:?!'\""
                        if add_txt[0] in punc: add_txt = add_txt[1:]
                        pending_strip_punctuation = False
                    
                    rebase_triggered = False
                    if d_cnt: utils.send_backspaces(d_cnt)
                    if add_txt: utils.type_text(add_txt)
                    synced_text = new_txt
                elif data.get('type') == 'reset':
                    synced_text = ""; pending_strip_punctuation = True
    finally:
...
def start_server_thread(loop, ui_callback):
    ...
    def on_pc_event(*args):
        global synced_text, rebase_triggered, pending_strip_punctuation
        if utils.is_typing() or rebase_triggered: return
        if synced_text and any(c.get('detect_keyboard') for c in client_configs.values()):
            synced_text = ""; rebase_triggered = True; pending_strip_punctuation = True
            for ws in list(connected_clients):
                asyncio.run_coroutine_threadsafe(ws.send_json({'type': 'rebase'}), loop)

    keyboard.Listener(on_press=on_pc_event).start()
    mouse.Listener(on_click=on_pc_event).start()
    loop.run_forever()